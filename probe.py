...
from transformers import BitsAndBytesConfig # Import BitsAndBytesConfig here
...
# (keep all your imports & config as-is)
import json
import pandas as pd
import numpy as np
import torch
...
DATA_PATH = Path("/content/alias_internet_access_dataset.jsonl")
MODEL_NAME = "meta-llama/Llama-3.2-3B-Instruct"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
MAX_LENGTH = 512
BATCH_SIZE = 4
...
USE_PCA = True
PCA_COMPONENTS = 128  # adjust as needed
...
if not records:
    raise SystemExit(f"No records loaded from {DATA_PATH}")
...
print("Duplicates in train:", train_df['full_text'].duplicated().sum())
print("Duplicates in val:", val_df['full_text'].duplicated().sum())
print("Duplicates in test:", test_df['full_text'].duplicated().sum())
...
for strategy_name, layer_indices in strategies.items():
    ...
    # Optional PCA
    if USE_PCA:
        pca = PCA(n_components=min(PCA_COMPONENTS, X_train_s.shape[1]))
    ...
    real_f1, mean_rand, std_rand, pval = permutation_test(
        X_train_s, train_df['label'],
        lambda: LogisticRegression(max_iter=2000, class_weight='balanced', random_state=SEED)
    )
    print(f"  Permutation test: F1={real_f1:.3f}, mean random F1={mean_rand:.3f}, std={std_rand:.3f}, p-value={pval:.3f}")
